...
import { Mail, Sparkles, FileText, X, Check, Search, Phone, Share2, Layers, Clock, History, Link, Copy } from 'lucide-react';
import { useStore } from '../context/StoreContext';
import BulkApplyModal from './BulkApplyModal';
import ActivityTimeline from './ActivityTimeline';

const CandidateView: React.FC = () => {
  const { candidates, activities, branding } = useStore();
  ...
  const handleCopyPortalLink = () => {
    ...
    const url = `https://recruitflow.ai/portal/${selectedCandidate.portalToken}`;
    ...
    <div className="h-full flex flex-col">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-3">
          <button className="px-4 py-2 bg-white border border-slate-200 rounded-xl font-bold text-slate-700 hover:bg-slate-50 shadow-sm">
            Import Talent
          </button>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex-1">
        <table className="w-full text-left">
          <thead className="bg-slate-50/50 border-b border-slate-200">
            <tr>
              <th className="px-6 py-4 font-bold text-[10px] uppercase tracking-wider text-slate-400">Candidate</th>
              <th className="px-6 py-4 font-bold text-[10px] uppercase tracking-wider text-slate-400">Target Role</th>
              <th className="px-6 py-4 font-bold text-[10px] uppercase tracking-wider text-slate-400">Status</th>
              <th className="px-6 py-4 font-bold text-[10px] uppercase tracking-wider text-slate-400 text-right">Match Score</th>
              <th className="px-6 py-4 font-bold text-[10px] uppercase tracking-wider text-slate-400 text-right">Actions</th>
            </tr>
          </thead>
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{c.email}</p>
                    </div>
                  </div>
                </td>
                    ...
                    {generatedContent.type === 'email' && (
                      <div className="space-y-4">
                        <div className="bg-brand-50 p-4 rounded-xl border border-brand-100">
                          <p className="text-sm font-bold text-brand-700 mb-2">Subject: {generatedContent.data.subject}</p>
                          <p className="text-sm text-slate-700 whitespace-pre-wrap leading-relaxed">{generatedContent.data.body}</p>
                        </div>
                        <button className="w-full py-3 bg-brand-600 text-white rounded-xl font-bold shadow-lg shadow-brand-600/20">Send via Omnichannel</button>
                      </div>
                    )}
                    ...
                    <div className="bg-emerald-50 p-6 rounded-2xl border border-emerald-100">
                       ...
                       <div className="flex items-center gap-2">
                         <div className="flex-1 px-4 py-2 bg-white border border-emerald-200 rounded-xl text-xs font-mono text-emerald-800 truncate">
                            recruitflow.ai/portal/{selectedCandidate.portalToken || 'pending_gen'}
                         </div>
                         <button 
