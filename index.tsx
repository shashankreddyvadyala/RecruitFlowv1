...
# Widgets persist per notebook. If you previously ran an older version of this file,
# uncomment the next line ONCE to clear stale widgets, then comment it out again.
# Leave it commented for scheduled job runs — it would wipe job-supplied parameters.
...
# Backticks are not optional: a catalog whose name starts with a digit
# (36383_something) is not a valid bare SQL identifier and USE CATALOG fails on it.
Q = lambda name: "`" + str(name).replace("`", "``") + "`"
FQ = lambda t: f"{Q(CATALOG)}.{Q(SCHEMA)}.{Q(t)}"
spark.sql(f"USE CATALOG {Q(CATALOG)}")
spark.sql(f"USE SCHEMA {Q(SCHEMA)}")
...
if RUN_MODE in ("mine_rules", "both"):
    ...
    if preds:
        ...
        if len(walk):
            walk["RUN_ID"] = RUN_ID
            write_delta(walk, FQ(ACC_TBL + "_walkforward"))
